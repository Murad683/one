...
export class AzureStorageProvider implements IStorageProvider {
  ...
  /**
   * Generates a write-only SAS URL for direct browser-to-Azure uploads.
   * The returned URL grants only create + write permissions (no read/delete).
   * Container is auto-created if it does not exist.
   */
  async generateUploadSasUrl(
    ...
    // Apply public access policy only for designated public containers
    const publicContainers = ['avatars', 'images', 'thumbnails', 'highlights'];
    if (publicContainers.includes(container)) {
      try {
        await containerClient.setAccessPolicy('blob');
      } catch {
        console.warn(`Could not update access policy for ${container}. Public access might be disabled on the Azure Storage Account level.`);
      }
    }
