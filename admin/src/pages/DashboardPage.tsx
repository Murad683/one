...
export const DashboardPage = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-heading">İdarəetmə Paneli</h1>
        <p className="mt-1 text-sm text-muted">Məzmun və müştəri aktivliyinə ümumi baxış.</p>
      </div>
      ...
      <section className="space-y-3">
        <div>
          <h2 className="text-lg font-semibold text-heading">Son Mesajlar</h2>
          <p className="text-sm text-muted">Əlaqə formasından gələn son müraCIətlər.</p>
        </div>
        <Table
          columns={columns}
          data={messages}
...
const StatCard = ({
  ...
  <div className="rounded-xl border border-edge bg-surface p-5 shadow-sm">
    <div className="flex items-center gap-4">
      <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-surface-hover text-body">
        <Icon className="h-5 w-5" />
      </div>
      <div>
        <p className="text-sm font-medium text-muted">{title}</p>
        {isLoading ? (
          <div className="mt-2 h-7 w-14 animate-pulse rounded bg-surface-hover" />
        ) : (
